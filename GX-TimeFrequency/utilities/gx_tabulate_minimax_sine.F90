...
!> Use: gx_tabulate_grids_sine.exe -help to list available options.
program gx_tabulate_minimax_sine
#include "gx_common.h"
    ...
    real(dp) :: sinft_duality_error, max_error_sin_wt
    ...
    real(dp), allocatable :: cosft_wt(:,:), cosft_tw(:,:), sinft_wt(:,:), sinft_tw(:,:)
    ...
    case ("table")
     ...
     write(msg, "(a)") &
      "num_points ierr cosft_duality_error max_err_costf_t_to_w max_err_costf_w_to_t max_err_sintf_t_to_w sinft_duality_error max_err_sintf_w_to_t eratio"
     ...
     do ii=1, size(tau_npoints_supported)
         ...
         call gx_minimax_grid(num_integ_points, emin, emax, &
                 ...
                 sinft_tw=sinft_tw,sinft_duality_error=sinft_duality_error, &
                 max_error_sin_wt=max_error_sin_wt)

         write(msg, "(2(i0,2x), *(es16.8,2x))") num_integ_points, ierr, cosft_duality_error, max_errors, &
                                                sinft_duality_error, max_error_sin_wt, eratio
         call dumps(msg, [std_out, unt])
     end do
     close(unt)
     ...
     'ynames = ["cosft_duality_error", "max_err_costf_t_to_w", "max_err_costf_w_to_t", "max_err_sintf_t_to_w", "sinft_duality_error", "max_err_sintf_w_to_t"] # "ierr"', &
    ...
    case ("print")
     ...
     write(msg,"(a)") &
             " mesh_index tau tau_weight omega omega_weight cosft_duality_error max_err_costf_t_to_w"// &
                     " max_err_costf_w_to_t max_err_sintf_t_to_w sinft_duality_error max_err_sintf_w_to_t eratio"
     call dumps(msg, [std_out, unt])
     do it=1,ntau
         write(msg, "(i0, *(es12.5,2x))")it, tau_mesh(it), tau_wgs(it), iw_mesh(it), iw_wgs(it), &
                 cosft_duality_error, max_errors, sinft_duality_error, max_error_sin_wt, eratio
         call dumps(msg, [std_out, unt])
     end do
     close(unt)
     ...
     call my_open("_gx_minimax_print_sintf_w_to_t.dat", unt)
     write(msg,"(3a, i0)")" sintf_w_to_t. sin(wt) already included. array shape: (nt, nw) ",ch10, " ntau: ", ntau
     call dumps(msg, [std_out, unt])
     do it=1,ntau
         write(msg, "(*(es12.5,2x))")sinft_tw(:, it)
         call dumps(msg, [std_out, unt])
     end do
     close(unt)
     ...
     'title = ""', &
     'for k in "cosft_duality_error max_err_costf_t_to_w max_err_costf_w_to_t max_err_sintf_t_to_w sinft_duality_error max_err_sintf_w_to_t eratio".split():', &
...
contains
    ...
    subroutine print_help()
        ...
        write(std_out, "(a,/)")"    gx_tabulate_grids_sine.exe print -ntau 6 -emin 0.4 -emax 100"
        write(std_out, "(a,/)")"To print all the meshes with erange=emax/emin, use:"
        write(std_out, "(a,/)")"    gx_tabulate_grids_sine.exe table -emin 0.4 -emax 100"
...
end program gx_tabulate_minimax_sine
